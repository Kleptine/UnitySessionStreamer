...
namespace Core.Streaming
{
    ...
    public class SessionStreamer : MonoBehaviour
    {
            ...
            Dictionary<string, string> metadata = null)
        {
            ...
            {
                logSize = new FileInfo(Application.consoleLogPath).Length;
            }
            ...
            {
                Debug.LogError($"(SessionStreamer) Session ID is null or whitespace. [{sessionId}]");
                return;
            }

            if (sessionId.IndexOfAny(Path.GetInvalidFileNameChars()) != -1)
            {
                Debug.LogError($"(SessionStreamer) Session ID contains invalid characters: [{sessionId}]");
                return;
            }
            
            if (sessionId.Contains(" "))
            {
                Debug.LogError($"(SessionStreamer) Session ID contains invalid spaces: [{sessionId}]");
                return;
            }
            ...
            try
            {
                ...
                streamer.sessionMetadata.Add("timestamp_utc",
                    XmlConvert.ToString(timestampNow, XmlDateTimeSerializationMode.Utc));
        ...
        private IEnumerator Start()
        {
            ...
            string url = hostUrl + "/whip?session_id=" + Uri.EscapeDataString(sessionId) + "&project_id=" + Uri.EscapeDataString(projectId);

            foreach (var (key, value) in sessionMetadata)
            {
                if (key == null)
                {
                    Debug.LogWarning($"(SessionStreamer) Session Metadata contains a null key.");
                    continue;
                }
                
                if (value == null)
                {
                    Debug.LogWarning($"(SessionStreamer) Session Metadata key [{key}] has null value.");
                    continue;
                }
                    ...
                    iceCandidates.Count > 1)
                {
                    ...
                    if (foundNonLocal)
                    {
                        ...
                        Debug.LogWarning(
                            ...
                            $" Attempting with [{iceCandidates.Count}] candidates:\n" + builder);
                        break;
                    }
                }
        ...
        private static bool IsLocalIpAddress(string ipAddressString)
        {
